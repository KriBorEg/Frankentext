...
char book[] = {
#embed "Frankenstein.txt" /// Stores the content of the file as an array of chars.
    , '\0'
};      /// Makes `book` a string.
...
void tokenize_and_fill_succs(char *delimiters, char *str) {
  ...
  while (token != NULL) {
    // If we have a previous token, add current token as its successor
    if (prev_token != NULL) {
...
size_t random_token_id_that_starts_a_sentence() {
  size_t attempts = 0;
  size_t max_attempts = tokens_size * 2; // Prevent infinite loop
  
  do {
    size_t random_id = rand() % tokens_size;
    // YOUR CODE HERE
    if (isupper(tokens[random_id][0])) {
      return random_id;
    }
    attempts++;
  } while (attempts < max_attempts);
  
  // Fallback: return first token that starts with capital letter
  ...
  // Ultimate fallback: return first token
  return 0;
}
...
char *generate_sentence(char *sentence, size_t sentence_size) {
  ...
  do {
    // YOUR CODE HERE
    // Get the number of successors for the current token
    size_t num_successors = succs_sizes[current_token_id];
    
    // If no successors, break out of loop
    if (num_successors == 0) {
      break;
    }
    
    // Choose a random successor
    size_t random_successor_index = rand() % num_successors;
    char *next_token = succs[current_token_id][random_successor_index];
    
    // Calculate length needed for next iteration
    sentence_len_next = strlen(sentence) + strlen(next_token) + 1; // +1 for space
    
    // If adding this token would exceed sentence size, break
    ...
    // If this token ends a sentence, we're done
